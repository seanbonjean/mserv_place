...
def count_mserv_recieve_dataflow(mservs:MicroService,users: User) -> map:    
    ...
    print(mserv_recieve_data_count)
    # 是不是少了个回到用户的数据发送量的考虑
...
def place_mserv(upper_bound:map, ksi: float,edge_nodes:list, mservs:list, users:list, channelrate_dict:list,mserv_user_count:map) -> map:
    ...
    for mserv in mservs:
        ...
        def allocate_node_to_group(around_node: int, single_node_group: list):
            ...
            while i < len(mserv_exits_nodes):
                ...
                if channelrate_dict[(around_node, unallocated_node)] > ksi:
                    ...
                    print(f"{single_node_group},{i},{mserv_exits_nodes[i]}")
            ...
            for node in single_node_group:
                ...
                for node2 in single_node_group:
                    ...
                    if channelrate_dict[(node, node2)] > max_rate:
                        ...
                        hub_node = node2
                if hub_node not in hub_nodes:
                    hub_nodes.append(hub_node)
        print(hub_nodes) # 枢纽节点
        ...
        print(allocate_mserv_result)
        mserv_process_info.append({
            'num':mserv.num, # 微服务序号
            'mserv_place_node_list':allocate_mserv_result, # 放置微服务的节点列表
            'hub_nodes':hub_nodes, # 枢纽节点列表
            'devide_node_group':node_group, # 节点群列表
            'allocate_mserv_to_node_group_num_result':allocate_mserv_to_node_group_result   # 节点群被分配的微服务数目
            })
...
if __name__ == '__main__':
    ...
    place_mserv(upper_bound,6,edge_nodes, mservs, users, channelrate_dict,mserv_user_count)