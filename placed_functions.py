...
def count_mserv_user(mservs:MicroService,users: User) -> dict:
...
def count_mserv_receive_dataflow(mservs:MicroService,users: User) -> dict:    
    # 统计每个微服务在各边缘节点接收数据量
    ...
    for user in users:
        for mserv_item_index in range(len(user.mserv_dependency)):
            if mserv_item_index==0:
                mserv_receive_data_count[user.mserv_dependency[mserv_item_index]][user.serv_node] += user.request_datasize
            else:
                mserv_receive_data_count[user.mserv_dependency[mserv_item_index]][user.serv_node] += mservs[user.mserv_dependency[mserv_item_index-1]].send_datasize
    # 是不是少了个回到用户的数据发送量的考虑
...
def place_mserv(upper_bound:dict, ksi: float,edge_nodes:list, mservs:list, users:list, channelrate_dict:list,channel_connectivity:dict,mserv_receive_data_count:dict ,mserv_user_count:dict) -> dict:
    ...
    for mserv in mservs:
        ...
        for i in range(edge_nodes_count):
            for j in range(edge_nodes_count):
                if i==j:
                    continue
                connected_weight_order_map[edge_nodes[i].num]+=channelrate_dict[(edge_nodes[i].num, edge_nodes[j].num)]
        ...
        connected_weight_order_map = dict(sorted(connected_weight_order_map.items(), key=lambda item: item[1], reverse=True))
        print("connected_weight_order_map 连通权值及其降序排列结果 (format: [{节点序号: 连通权值}, ...]): ",connected_weight_order_map)
        ...
        for single_node_group in node_group:
            ...
            for connected_node_index in range(edge_nodes_count):
                ...
                if len(connectivity_node_list)>=3:
                    ...
                    hub_node_tran_time=sum([mserv_receive_data_count[mserv.num][other_node]/channelrate_dict[(connected_node_index, other_node)] for other_node in connectivity_node_list])
                    for other_node in connectivity_node_list:
                        delta = hub_node_tran_time - sum([mserv_receive_data_count[mserv.num][group_node]/channelrate_dict[(group_node, other_node)] for group_node in connectivity_node_list if group_node!=other_node])
                        if delta<0:
                            single_node_group.append(connected_node_index)
                            hub_nodes.append(connected_node_index)
                            break
        print("hub node 枢纽节点列表: ",hub_nodes) # 枢纽节点
        print("node group after add hub nodes (加入枢纽节点后的节点群列表，其中单个元素为组成对应节点群的节点的列表): ",node_group)
...
if __name__ == '__main__':
    ...
    print("mserv_user_count (微服务用户数统计。二维字典，第一维是微服务种类序号，第二维是以边缘节点序号): ",{k: (dict(v) if isinstance(v, defaultdict) else {kk: (dict(vv) if isinstance(vv, defaultdict) else vv) for kk, vv in v.items()} if isinstance(v, dict) else v) for k, v in mserv_user_count.items()})
    mserv_receive_data_count = count_mserv_receive_dataflow(mservs,users)
    print("mserv_receive_data_count (微服务数据接收量统计。二维字典，第一维是微服务种类序号，第二维是以边缘节点序号): ",{k: (dict(v) if isinstance(v, defaultdict) else {kk: (dict(vv) if isinstance(vv, defaultdict) else vv) for kk, vv in v.items()} if isinstance(v, dict) else v) for k, v in mserv_receive_data_count.items()})
    upper_bound = {0:2,1: 2, 2: 4, 3: 3, 4: 2, 5: 3, 6: 3, 7: 3,8:5,9:3,10:5,11:4,12:2,13:2,14:4}
    ksi=0.6 #通信速率阈值 ξ
    place_mserv(upper_bound=upper_bound,ksi=ksi,edge_nodes=edge_nodes, mservs=mservs, users=users, channelrate_dict=channelrate_dict,channel_connectivity=channel_connectivity,mserv_receive_data_count=mserv_receive_data_count,mserv_user_count=mserv_user_count)